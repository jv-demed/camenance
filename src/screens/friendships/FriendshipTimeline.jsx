...
function buildPath(numRows, W) {
    ...
    const Y = LINE_Y;
    const R = CORNER_R;
    const P = 4; // Padding (afasta das bordas invisíveis para não cortar a linha)
    const LEFT = P;
    const RIGHT = W - P;
    ...
    return (
        <div className='relative flex flex-col items-center gap-1 shrink-0 group'>
            ...
            {isLoadingPhoto ? (
                ...
                <div className='relative flex flex-col items-center mt-2 group/photo'>
                    {/* Seta (Triângulo) apontando para a bolinha */}
                    <div className='absolute -top-1.5 w-3 h-3 bg-white border-t border-l border-gray-200 rotate-45 z-0'></div>
                    
                    {/* Container da foto (estilo moldura) */}
...
export function FriendshipTimeline({ encounters = [], friends = [], photosByEncounter = {}, loading, onEditEncounter }) {
    ...
    return (
        <div ref={containerRef} className='relative w-full pr-[100px]' style={{ minHeight: rows.length * ROW_HEIGHT }}>
            ...
            {rows.map((rowEncs, rowIdx) => {
                ...
                return (
                    ...
                    >
                        {/* Coluna de legenda lateral */}
                        <div className='absolute right-[-100px] top-0 w-[100px] h-full border-l border-gray-100 pointer-events-none'>
                            {showLabel && (
                                <span className='absolute left-1/2 -translate-x-1/2 text-gray-400 text-[11px] font-semibold tracking-wider whitespace-nowrap' 
                                      style={{ top: LINE_Y - 8 }}>
                                    {rowDateLabel}
                                </span>
                            )}
